...
3. Enter the displayed **multiplier**, not a session count. Confirm Academy versus Reward from game evidence. White/grey is auto-confirmed from the selected player’s persisted established roles and current stat state; rows are labelled **ROLE STATE**. Tap a row only when direct game evidence requires a manual override.
...
5. Enter the game's actual low/high preview bounds, plus the separately displayed OVR boost if available. If the displayed starting stats and role-derived white/grey classes match the stored player state, the app treats that state as confirmed automatically and enables saving; there is no redundant confirmation button. Scanned ranges are prefilled only while player, coach label, multiplier and affected-stat set still match the scan. Unknown rows can remain blank. Observations are never averaged.
