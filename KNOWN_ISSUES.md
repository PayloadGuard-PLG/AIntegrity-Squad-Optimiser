...
| 2 | XP calibration | `baseXpPerSession: 150` is a working estimate. Needs empirical validation — note a player's stat value before a known number of sessions, compare observed % gain to engine output, adjust value in `profiles/game_2025.json` accordingly. | High |
